...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('activating extension "macgpt"');
	// Get the settings from the extension's configuration
	const config = vscode.workspace.getConfiguration('macgpt');
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('macgpt.ask', () => 
			vscode.window.showInputBox({ prompt: 'What do you want to do?' })
			.then((value) => provider.search(value))
		),
		vscode.commands.registerCommand('macgpt.explain', () => commandHandler('promptPrefix.explain')),		
		vscode.commands.registerCommand('macgpt.refactor', () => commandHandler('promptPrefix.refactor')),
		vscode.commands.registerCommand('macgpt.optimize', () => commandHandler('promptPrefix.optimize')),
		vscode.commands.registerCommand('macgpt.Codeinspection', () => commandHandler('promptPrefix.Codeinspection')),
		vscode.commands.registerCommand('macgpt.documentation', () => commandHandler('promptPrefix.documentation')),
		vscode.commands.registerCommand('macgpt.generateunittest', () => commandHandler('promptPrefix.generateunittest')),
		vscode.commands.registerCommand('macgpt.seleniumtestcase', () => commandHandler('promptPrefix.seleniumtestcase')),
		vscode.commands.registerCommand('macgpt.reacttestcase', () => commandHandler('promptPrefix.reacttestcase')),
		//vscode.commands.registerCommand('macgpt.converttohtml', () => commandHandler('promptPrefix.converttohtml')),
		vscode.commands.registerCommand('macgpt.resetConversation', () => provider.resetConversation())
	);


	// Change the extension's session token or settings when configuration is changed
	vscode.workspace.onDidChangeConfiguration((event: vscode.ConfigurationChangeEvent) => {
		if (event.affectsConfiguration('macgpt.apiKey')) {
			const config = vscode.workspace.getConfiguration('macgpt');
			provider.setAuthenticationInfo({apiKey: config.get('apiKey')});
		}else if (event.affectsConfiguration('macgpt.apiUrl')) {
			const config = vscode.workspace.getConfiguration('macgpt');
			let url = config.get('apiUrl')as string || BASE_URL;
			provider.setSettings({ apiUrl: url });
		} else if (event.affectsConfiguration('macgpt.model')) {
			const config = vscode.workspace.getConfiguration('macgpt');
			provider.setSettings({ model: config.get('model') || 'gpt-3.5-turbo' }); 
		} else if (event.affectsConfiguration('macgpt.selectedInsideCodeblock')) {
			const config = vscode.workspace.getConfiguration('macgpt');
			provider.setSettings({ selectedInsideCodeblock: config.get('selectedInsideCodeblock') || false });
		} else if (event.affectsConfiguration('macgpt.codeblockWithLanguageId')) {
			const config = vscode.workspace.getConfiguration('macgpt');
			provider.setSettings({ codeblockWithLanguageId: config.get('codeblockWithLanguageId') || false });
		} else if (event.affectsConfiguration('macgpt.pasteOnClick')) {
			const config = vscode.workspace.getConfiguration('macgpt');
			provider.setSettings({ pasteOnClick: config.get('pasteOnClick') || false });
		} else if (event.affectsConfiguration('macgpt.keepConversation')) {
			const config = vscode.workspace.getConfiguration('macgpt');
			provider.setSettings({ keepConversation: config.get('keepConversation') || false });
		} else if (event.affectsConfiguration('macgpt.timeoutLength')) {
			const config = vscode.workspace.getConfiguration('macgpt');
...
class ChatGPTViewProvider implements vscode.WebviewViewProvider {
	public static readonly viewType = 'macgpt.chatView';
				...
				case 'codeSelected':
					{
						...
						vscode.commands.executeCommand(
							'vscode.openWith',
							vscode.Uri.parse("untitled:MacGPT"),
	...
	public async search(prompt?:string) {
		...
		if (!this._view) {
			await vscode.commands.executeCommand('macgpt.chatView.focus');
		} else {
	...
	private _getHtmlForWebview(webview: vscode.Webview) {
		...
		return `<!DOCTYPE html>
			...
			<body>
				<input class="h-10 w-full text-white bg-stone-700 p-4 text-sm" placeholder="Ask MacGPT something" id="prompt-input" />
